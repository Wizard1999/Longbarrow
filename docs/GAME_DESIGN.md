...
These surfaced from a full pass through every source document looking for ideas that were mentioned once and never resolved or re-flagged. None of these have been decided — captured here so they aren't lost before their relevant build phase.

- **Gatherable resource(s) — the core economic input.** Undecided: is there one universal gatherable resource across all four races, or multiple (e.g. a StarCraft-style two-resource system)? Related terms floated earlier in design conversations — "Essence" (an early, since-abandoned Conclave-specific resource name from the original pre-Claude conversation) and "Dominion" (mentioned once, never defined — see the Relic tech / Dominion entry above) — were never adopted as part of a resolved system and should not be assumed to still apply unless explicitly revived. This is a hard dependency of Phase 1 step 1.4 (Resource Nodes & Worker Gather Loop) and needs a decision before or during that step.
- **Map geometry — tunnels, ramps, hidden routes.** An early rejected *theme* direction ("living playset warfare") contained a separate mechanical idea worth keeping on the table independent of the theme it was attached to: literal terrain features like tunnels, ramps, and hidden/alternate routes as first-class map elements, beyond the general "terrain matters" pillars (high ground, flanking, chokepoints) already locked in Section 2. Unclear whether this is redundant with existing terrain rules or a distinct system. Relevant to Phase 1 step 1.3 (Terrain & High Ground) and later concrete map design.
- **Stealth / detection.** The unit rosters (Section 8.7) already assume this system exists — Cohort's Chronicler "reveals stealth," Conclave's Phantom has "illusions/stealth" — but no stealth or detection mechanic is defined anywhere in the design doc. This is a dependency hiding inside the roster table rather than a standalone idea, and needs resolving by the time those units are implemented (Phase 1 for Chronicler, Phase 3 for Phantom).
- **Air units' relationship to the supply/automation resource.** Section 7 lists "command bandwidth" as one of the mechanisms discouraging deathball air, but it's never explicitly tied back to the Section 8.3 supply-gates-automation table (Command / Population / Coordination / Territory). Undecided whether air squads draw from the same per-race pool as ground squads or something separate. Not relevant until Phase 4.2 (Air Units) but worth deciding well before then since it affects the supply system's design.
