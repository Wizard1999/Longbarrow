...
These surfaced from a full pass through every source document looking for ideas that were mentioned once and never resolved or re-flagged. **All four are now resolved** (D-031–D-035); each entry below records the answer and points at the decision carrying the reasoning.

- **Gatherable resources — RESOLVED, D-031.** Two gathered currencies: **Material** (common, plentiful, builds and trains) and **Legacy** (rare, contested, few nodes, buys research and advanced units — violet per D-025). **Dominion** is deferred to a later domain/territory system rather than being a currency, and **Relics** are deferred until PvP is settled. The split must never require per-worker allocation: the player's economic decision is which nodes to *hold*, not which worker mines what. Migration from the current single `essence` is queued in `TODO.md` and must land atomically.
- **Map geometry — RESOLVED, D-033 and D-035.** Not a distinct system: ramps are heightfield gradients, chokepoints are boundary narrowing, hidden routes are fog plus an unwatched path. The work is **generation**, not mechanics — tunnels, ramps and alternate routes must arise innately from the generator, which must produce legible approaches and flank routes rather than smooth noise. Generated toward an **arcade** target (D-035): *Halo*/*Quake* rather than *Battlefield* — fun, exploratory and readable, explicitly not realistic. Explicit tunnel entities stay deferred; solve it in generation first.
- **Stealth / detection — RESOLVED, D-032.** Terrain concealment only; no permanently invisible units. Cover conceals whoever stands in it from enemies beyond a short detection radius, and moving fast or attacking reveals you. Detection is a radius every unit has; the Chronicler widens it rather than granting a binary reveal. Declared as a terrain trait the engine reads generically (D-029), so the Chronicler is buildable now and Conclave's Phantom becomes illusion-and-cover in Phase 3.
- **Air units' relationship to the supply/automation resource — RESOLVED, D-034.** One pool for every unit, air included. Shared supply *is* the anti-deathball mechanism: a separate air cap would make air additive, which is how deathballs form. §8.3 remains the single lever, and Phase 4.2 needs no structural change to the supply system.

---

*All four §11.1 gaps were resolved by the designer on 2026-07-27 — see D-031 through D-035. The text above records each answer inline; the decisions carry the reasoning and the rejected alternatives.*

*Section 11.1 added following a full audit of `01_design_document.md`, `02_development_blueprint.md`, `03_chat_transcript.md`, and `04_original_reference_conversation.md` to identify ideas that were mentioned once and never carried forward or explicitly marked as dropped. Nothing in 11.1 had been decided at the time of that audit; all four have since been resolved (D-031–D-035). Restored 2026-07-27 after being dropped in an intermediate revision.*
